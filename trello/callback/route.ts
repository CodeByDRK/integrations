// app/api/trello/callback/route.ts
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const clientSecret = process.env.ALTASSIAN_DEVELOPER_CLIENT_SECRET;
    ...
    const userInfoUrl = "https://api.trello.com/1/members/me";
    const userInfoResponse = await axios.get(userInfoUrl, {
      headers: { Authorization: `Bearer ${access_token}` },
    });

    console.log("Trello User Info Response:", userInfoResponse.data);

    const trelloUserId = userInfoResponse.data.id;

    // Store the token in the database
    let integration = await prisma.integration.findFirst({
      where: { userId, integrationType: "TRELLO" },
    });
    ...
    const successMessage = `<html><body><div>Trello connected successfully</div></body></html>`;
